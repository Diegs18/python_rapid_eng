...
precisions = [0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001, 0.0000001] #list of precision of pi we want to check for
for prec in precisions:
    success_cnt = 0
    sum_pi = 0
    for j in range(1,101): #100 attempts of calculating pi
        pi_cnt = 0
        ave_pi = 0
        for i in range(1, 10001): #10,000 samples of trying to calculate pi
            x = random()
            y = random()
            z = np.sqrt(x**2 + y**2)
            if z<1: #do we count the value?
                pi_cnt += 1
            calc_pi = (4*pi_cnt) / i
            if abs(np.pi - calc_pi) < prec: #is our cal pi close enough?
                sum_pi += calc_pi
                success_cnt += 1
                break; #do next attampt
    if(success_cnt<1):
